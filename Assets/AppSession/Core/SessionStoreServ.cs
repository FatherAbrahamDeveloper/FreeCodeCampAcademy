...
public class DistributedSessionStoreWithStart : ISessionStore
{
    ...
    public DistributedSessionStoreWithStart(IDistributedCache cache, ILoggerFactory loggerFactory, IStartSession startSession)
    {
    ...
    {
        ISession session = innerStore.Create(sessionKey, idleTimeout, ioTimeout, tryEstablishSession, isNewSessionKey);
            if (isNewSessionKey)
        {
